"""Apply the manual second-pass review to golden_set/golden_250.csv.

Review method: all 200 rows read individually in a dev session.
- Intent judged against src/intent_taxonomy.py (catch-all -> GENERAL_COMPLAINT).
- Escalate judged against the fixed human definition below (NOT the LLM
  labeller's loose criteria).

Human ESCALATE=True definition (fixed BEFORE labelling):
  1. legal: lawsuit/lawyer/sue/court/regulator (FTC/BBB)/chargeback/dispute-charge
  2. fraud/security: fraud charge, unauthorized order/transaction, hacked
     account, phishing-suspicious email
  3. safety: self-harm, threats, emergency
  4. sensitive data posted publicly: card/SSN/CVV or Amazon order-ID pattern
  5. repeat-contact failure: explicit mention of prior failed support attempts
     ("contacted twice", "third call", "still no update", "ignoring my issue",
     "like talking to a robot", failed callback/helpline, broken promises)
  6. explicit request for a human / rejection of automated handling
  7. genuinely uninterpretable
Otherwise AUTO (angry venting alone, profanity alone, urgency alone -> auto).

Usage: python apply_review.py
"""
...
def main() -> None:
    path = Path("golden_set/golden_250.csv")
    df = pd.read_csv(path)
