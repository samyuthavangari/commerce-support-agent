"""Evaluate the 50 top-up rows through the live agent + judge, append results.

Golden is now 250 rows; agent_replies.csv / judge_scores.csv have 200.
This fills the gap (~4 min API) so a full re-run is unnecessary.
"""
...
def main() -> None:
    ...
    replies.to_csv("results/agent_replies.csv", index=False)

    judged = pd.read_csv("results/judge_scores.csv")
    ...
    judged.to_csv("results/judge_scores.csv", index=False)
