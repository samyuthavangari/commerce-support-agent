...
Customer message
      ↓
Intent classifier (LLM, JSON) → raw confidence → calibrate_confidence() (stored, not yet gating)
      ↓
Qdrant retrieval (intent-filtered top-5 + similarity scores)
      ↓
Drafter (RAG prompt, 280-char + deterministic URL guardrail)
      ↓
Escalation v5 (deterministic rules: legal / PII / safety / fraud /
public-PII / repeat-contact / human-ask / unclear / thread>=5 / conf<0.45
+ narrow LLM tiebreaker for ultra-short <=4-token messages)
      ↓
Final Output: [AUTO-HANDLE] vs [ESCALATE TO HUMAN] + Reason Code
```
...
```
Calibration Split (cal_100.csv + cal2_100.csv, N=200)
      ↓
Optimize Thresholds & Derive Isotonic Calibration Map
      ↓
LOCK POLICY (v5 rules + tiebreaker, frozen per docs/escalation_v5.md)
      ↓
Untouched Test Sets (golden_250, heldout_50, heldoutB_50, stress_test_50)
      ↓
Report Final Safety Metrics (golden 0.981 / heldout-A 0.733 / heldout-B 0.364 / stress-demo 0.927)
```
