# Commerce Support Agent — Technical Evaluation Report

## 1. Executive summary
Provenance: all numbers below are generated by `run_eval.py` (`results/eval_report.json`, 2026-09-10T13:39:27Z, `gemini-3.1-flash-lite`, seed 42) + `docs/escalation_v5.md` frozen 5-set + `results/stress_locked_eval_results.csv` (`demo.py` locked policy). Shipped system = `src/agent.py::run_agent` (v5 rules + tiebreaker). `demo.py::run_demo_pipeline` adds experimental precedent/grounding/calibrated layers on top — not the evaluated harness.
**Primary Headline Metric & Deployment Gate**: **Held-Out Escalation Recall: 0.733 (Heldout-A) / 0.364 (Heldout-B)**. While in-distribution golden-250 achieves **0.981** recall (52/53 caught, 1 false-auto), out-of-distribution frozen held-outs expose severe regex coverage drops (12 false-autos across 350 combined cases). **Explicit engineering posture: We would not ship this system to production without closing this held-out gap** via semantic vector gating.
...
- **Golden Benchmark (in-distribution N=250, 53 pos)**: Recall **0.981** (52/53 caught, 1 false-auto, precision 0.867) — `eval_report.json`
...
- Holdout-A-50 (frozen): Recall **0.733**, Precision **0.733**, False-auto **0.267**
- Holdout-B-50 (frozen): Recall **0.364**, Precision **0.800**, False-auto **0.636**
- Golden-250 (in-distribution): Recall **0.981**, Precision **0.867**, False-auto **0.019**
- Stress-50 (`demo.py` extra layers): Recall **0.927** (38/41), 5 FPs

**Product Rationale & Shipment Gate**: In enterprise customer support, a false negative (auto-replying to an unauthorized charge, legal threat, or safety case) costs far more than a false positive (over-escalating to human triage). We intentionally bias toward escalation. However, **we would not ship this system without closing this held-out generalization gap**: heldout-B proves the current regex policy misses out-of-vocabulary fraud and billing paraphrases (`charged twice`, `fladuent` typo). Closing this via semantic vector gating is required before production deployment.

## 9. Evaluation methodology
Golden 250 (human-reviewed; intent κ=0.939 (LLM-vs-human-reviewer agreement, single reviewer, no adjudication pass) on first 200) + cal-100 + cal2-100 (tuning only) + frozen held-out A/B 50+50 + adversarial stress test 50; fair B1 (TF-IDF, 8k weak-labelled non-golden threads, seed 42); bootstrap CIs; provenance block per run. Full trace per example in `results/agent_replies.csv`.
...
82.0% accuracy rests on 250 single-brand 2017 tweets with single-reviewer labels (κ=0.939 LLM-vs-human-reviewer agreement, single reviewer, no adjudication pass) — and the rare-class top-up *lowered* it from 0.88, which is what honest stratification costs. Escalation 0.981 is in-distribution only: frozen heldout-B is 0.364 (false-auto 0.636) and heldout-A is 0.733 — quoting golden alone is cherry-picking, and we explicitly would not ship this system without closing this held-out gap. Raw confidence (mean 0.947) runs ~15 points above true accuracy, and `calibrate_confidence()` is stored but NOT gating in shipped code. The 24.44/25 reply score runs ~+4 above blind human scores with near-zero rank agreement (ρ=-0.062, exact 2/70). 52.4% auto-rate is a calibration-sweep optimum, not deployment coverage. Averages hide the dangerous tail: `fladuent` typo, `charged twice` paraphrase, and sarcastic delay all auto-handle today.
