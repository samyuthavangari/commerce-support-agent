"""
scripts/golden_set/recompute_250.py
───────────────────────────────────
Recomputes class balance, escalation rates, and metadata summaries for golden_250.csv.
When run: Executed to verify dataset integrity and class distributions.
...
def main() -> None:
    ...
    agreement = {}
    if Path(ROOT / "results" / "human_scores.csv").exists():
        agreement = compute_agreement(judge, Path(ROOT / "results" / "human_scores.csv"))
        agreement["n"] = 40
