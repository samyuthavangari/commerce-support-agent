"""
scripts/golden_set/merge_topup.py
─────────────────────────────────
Merges 50 rare-class top-up rows (PRIME, DEVICE) into golden_250.csv (200 -> 250 rows).
When run: Executed to correct keyword stratification imbalance with non-corpus samples.
Reproducibility: Repeatable dataset assembly script.
"""
...
def main() -> None:
    cand = pd.read_csv(ROOT / "golden_set" / "topup_texts.csv").set_index("message_id")
    ...
    assert len(out) == 250
    out.to_csv(ROOT / "golden_set" / "golden_250.csv", index=False)
